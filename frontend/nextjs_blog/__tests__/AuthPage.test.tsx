...
const handlers = [
  rest.post(
    `${process.env.NEXT_PUBLIC_RESTAPI_URLB}/jwt/create/`,
    (req, res, ctx) => {
      return res(ctx.status(200), ctx.json({ access: '123xyz' }))
    }
  ),
  rest.post(
    `${process.env.NEXT_PUBLIC_RESTAPI_URLB}/register/`,
    (req, res, ctx) => {
...
describe('AdminPage test cases', () => {
  ...
  it('Should route to index-page when register+login succeed', async () => {
    const { page } = await getPage({
      route: '/admin-page',
    })
    render(page)
  ...
  it('Should not route to index-page when register failed', async () => {
    server.use(
      rest.post(
        `${process.env.NEXT_PUBLIC_RESTAPI_URLB}/register/`,
        (req, res, ctx) => {
