...
process.env.NEXT_PUBLIC_RESTAPI_URL = 'http://blog-prj:8000/api'
process.env.NEXT_PUBLIC_RESTAPI_URLB = 'http://127.0.0.1:8000/api'
...
describe('BlogPage Test Cases', () => {
  it('Should route to AdminPage and route back to BlogPage', async () => {
  ...
  it('Should render delete btn + logout button when JWT token cookie exist', async () => {
  ...
  it('Should not render delete btn + logout button when JWT token cookie not exist', async () => {
    const { page } = await getPage({
      route: '/',
    })
    render(page)
  ...
  it('Should render the list of blogs by getStaticProps', async () => {
    const { page } = await getPage({
      route: '/',
    })
    render(page)
